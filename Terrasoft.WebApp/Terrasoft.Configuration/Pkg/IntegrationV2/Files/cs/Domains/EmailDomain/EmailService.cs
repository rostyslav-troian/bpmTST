namespace Terrasoft.EmailDomain
{
	...
	public class EmailService : IEmailService
	{
		...
		private List<AttachmentModel> CreateAttachments(Email emailDto, out string fixedBody) {
			...
			foreach (var attach in emailDto.Attachments) { 
				...
				if (attach.IsInline) {
					...
					fixedBody = fixedBody.Replace("cid:" + attach.Id, url);
				}
		...
		public void Send(Email email, Credentials credentials) {
			...
			emailProvider.Send(email, email.Attachments, credentials);
		}
