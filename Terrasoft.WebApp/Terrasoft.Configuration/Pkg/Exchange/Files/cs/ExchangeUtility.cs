namespace Terrasoft.Configuration
{
	...
	public class ExchangeUtilityImpl: IExchangeUtility
	{
		...
		/// <summary>
		/// SysSetting with code IgnoreExchangeSSLWarnings value.
		/// </summary>
		private bool _ignoreSslWarnings;
		...
		private Exchange.ExchangeService CreateExchangeService(UserConnection userConnection, ExchangeCredentials credentials, 
				string senderEmailAddress, bool stopOnFirstError = false) {
			ServicePointManager.ServerCertificateValidationCallback += ValidateRemoteCertificate;
			Exchange.ExchangeService exchangeService;
			try {
				var createExchangeServiceWithTimezone = userConnection.GetIsFeatureEnabled("CreateExchangeServiceWithTimezone");
				exchangeService = createExchangeServiceWithTimezone 
					? new Exchange.ExchangeService(Exchange.ExchangeVersion.Exchange2010_SP1, TimeZoneInfo.Utc)
					: new Exchange.ExchangeService(Exchange.ExchangeVersion.Exchange2010_SP1);
		...
		private Dictionary<string, Dictionary<Guid, string>> GetAttachmentsDictionary(
				UserConnection userConnection, string userEmailAddress) {
			var result = new Dictionary<string, Dictionary<Guid, string>>();
			Query select = null;
			...
			select = select.Where("AF", "Uploaded").IsEqual(Column.Parameter(false))
				...
				.And("A", "UserEmailAddress").IsEqual(Column.Parameter(userEmailAddress));
				...
				using (IDataReader reader = dbExecutor.ExecuteReader(select.GetSqlText(), select.Parameters)) {
					while (reader.Read()) {
						...
						if (!result.ContainsKey(emailMessageId)) {
							result[emailMessageId] = new Dictionary<Guid, string> { { activityFileId, attachmentId } };
						} else {
							result[emailMessageId].Add(activityFileId, attachmentId);
						}
					}
				}
			}
		...
		private Guid GetProcessLogRecordId(UserConnection userConnection, ProcessSchema processSchema) {
			...
			var select = new Select(userConnection).Top(1)
						...
						.Where("SPL", "SysWorkspaceId").IsEqual(Column.Parameter(userConnection.Workspace.Id))
							...
							.OrderBy(OrderDirectionStrict.Descending, "SPL", "CreatedOn");
			using (DBExecutor dbExecutor = userConnection.EnsureDBConnection()) {
				using (IDataReader reader = dbExecutor.ExecuteReader(select.GetSqlText(), select.Parameters)) {
					if (reader.Read()) {
		...
		private void UploadAttachmentData(UserConnection userConnection, KeyValuePair<Guid, string> attachment,
				IExchangeAttachmentUtilities attachmentHelper, Exchange.EmailMessage message) {
			string attachmentId = attachment.Value;
		...
		public List<ExchangeMailFolder> GetHierarhicalFolderList(Exchange.ExchangeService service,
			...
			Exchange.Folder folder = Exchange.Folder.Bind(service, folderName);
		...
		public List<ExchangeMailFolder> GetHierarhicalFolderList(Exchange.Folder rootFolder,
			...
			foreach (Exchange.Folder folder in folders) {
				...
				result.Add(new ExchangeMailFolder {
					...
					Selected = selectedFolders == null ? false : selectedFolders.Contains(folder.Id.UniqueId)
				});
			}
			return result;
		}
		...
		public Exchange.ExchangeService CreateExchangeService(UserConnection userConnection,
			...
			_ignoreSslWarnings = (bool)Terrasoft.Core.Configuration.SysSettings.GetValue(userConnection, "IgnoreExchangeSSLWarnings", false);
			string logTemplate = "ExchangeClient.CreateExchangeService(UserConnection , senderEmailAddress) -> " +
							...
							" and create ExchangeService and IgnoreExchangeSSLWarnings = '{3}'.\r\n";
			Log.InfoFormat(logTemplate, exchangeCredentials.ServerAddress,
					((Terrasoft.Mail.Credentials)(exchangeCredentials)).UserName, senderEmailAddress, _ignoreSslWarnings);
		...
		public void UploadAttachmentsData(UserConnection userConnection, string userEmailAddress) {
			...
			Dictionary<string, Dictionary<Guid, string>> attachmentsDictionary =
			...
			foreach (KeyValuePair<string, Dictionary<Guid, string>> email in attachmentsDictionary) {
				...
				try {
					IEnumerable<Exchange.Attachment> attachments = attachmentHelper.GetAttachments(message);
				} catch (Exchange.ServiceResponseException) {
					email.Value.ForEach(att => RaiseAttachmentError(userConnection, "Message attachments does not exist",
						att.Key));
					continue;
				}
				foreach (KeyValuePair<Guid, string> attachment in email.Value) {
					UploadAttachmentData(userConnection, attachment, attachmentHelper, message);
				}
			}
			ServicePointManager.ServerCertificateValidationCallback -= ValidateRemoteCertificate;
		}
