namespace Terrasoft.Sync.Exchange
{
	...
	public class ExchangeEmailMessage : ExchangeBase
	{
		...
		private SyncEntity GetActivityInstanceSync(SyncContext context, Exchange.EmailMessage message, string subject,
			...
			List<Guid> emailIds = ActivityUtils.GetExistingEmaisIds(context.UserConnection, sendDate, subject,
				message.Body.Text, context.UserConnection.CurrentUser.TimeZone);
			SyncAction action = SyncAction.None;
			if (context.UserConnection.GetIsFeatureEnabled("MailboxRightsForEmail")) {
				...
				action = activityExists ? SyncAction.None : SyncAction.Create;
			} else {
				if (emailIds.IsEmpty() || emailIds.All(id => !activity.FetchFromDB(id, false))) {
					string messageTpl = emailIds.IsEmpty() ? "Email {0} not found in DB." : "Email {0} exists, but user {1} has no read rights for record.";
					LogInfo(context, SyncAction.None, SyncDirection.Download, messageTpl, subject, context.UserConnection.CurrentUser.Id);
					activity.SetDefColumnValues();
		...
		/// <returns>Ture if message is incoming.</returns>
		/// <remarks>
		/// External dependency allocation.
		/// </remarks>
		protected virtual bool IsIncommingMessage(SyncContext context, Exchange.EmailMessage emailMessage) {
			...
			if (!emailMessage.IsIncoming() && !context.UserConnection.GetIsFeatureEnabled("MailboxRightsForEmail")) {
				var contactExtendedPropertyName = ExchangeUtility.GetContactExtendedPropertyName();
				IEnumerable<Exchange.ExtendedProperty> contactExtendedProperty =
					emailMessage.ExtendedProperties.Where(e => e.PropertyDefinition.Name == contactExtendedPropertyName);
				if (contactExtendedProperty.Any(e => Guid.Parse(e.Value.ToString()) ==
		...
		protected bool IsMessageLockedForSync(SyncContext context) {
			if (!context.UserConnection.GetIsFeatureEnabled("AsyncEmailEventHandling")) {
				return false;
			}
