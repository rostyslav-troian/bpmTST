...
( function() {
	...
	function getPillarAtPosition( pillars, positionX ) {
		for ( var i = 0, len = pillars.length; i < len; i++ ) {
			...
			if ( positionX >= (pillar.x - pillar.width) && positionX <= ( pillar.x + pillar.width ) )
				return pillar;
		}
	...
	function columnResizer( editor ) {
		...
		resizer = CKEDITOR.dom.element.createFromHtml( '<div data-cke-temp=1 contenteditable=false unselectable=on ' +
			'style="position:absolute;cursor:col-resize;filter:alpha(opacity=0);opacity:0;' +
				'padding:0;background-color:#004;background-image:none;border:0px none;z-index:1061"></div>', document );
		...
		this.attachTo = function( targetPillar ) {
			...
			resizer.setStyles( {
				...
				left: pxUnit( targetPillar.x - targetPillar.width ),
	...
	CKEDITOR.plugins.add( 'tableresize', {
		requires: 'bpmonlinetabletools',

		init: function( editor ) {
			editor.on( 'toggle_maximized', function() {
			...
			editor.on( 'contentDom', function() {
				...
				editable.attachListener( editable.isInline() ? editable : editor.document, 'mousemove', function( evt ) {
					...
					if ( !( pillars = table.getCustomData( '_cke_table_pillars' ) ) || this.toggleMaximized ) {
