Terrasoft.configuration.Structures["Highcharts"] = {innerHierarchyStack: ["Highcharts"]};
// ==ClosureCompiler==
...
// JSLint options:
/*jshint ignore:start*/
/*global Highcharts, document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location,
 jQuery, $, console, each, grep */
...
// default adapters below.
		...
		defaultOptions = {
			colors: ['#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5', '#c42525',
				'#a6c96a'], // docs
			...
			chart: {
				...
				spacingLeft: 10,
				style: {
					fontFamily: '"Bpmonline Open Sans", sans-serif', // default font
					fontSize: '14px'
				},
	 ...
	 */
		...
		Pointer.prototype = {
			...
			onDocumentMouseMove: function(e) {
				...
				if (chartPosition && hoverSeries && hoverSeries.isCartesian && !chart.isInsidePlot(e.pageX - chartPosition.left - chart.plotLeft,
		...
		Chart.prototype = {
			...
			getChartSize: function() {
				...
				chart.containerWidth = Ext.get(renderTo).getWidth();
				chart.containerHeight = Ext.get(renderTo).getHeight();
				// BrowserSupport: IE, FF ######## ###### ######## # ####### ############## ######.
				var chartWidth = optionsChart.width || chart.containerWidth || 600;
				chart.chartWidth = chartWidth;
				var titleHeight = 50;
				var renderToEl = Ext.get(renderTo);
				var parentEl = renderToEl.dom.parentElement;
				var childrenCount = parentEl.childElementCount;
				if (childrenCount >= 3) {
					var breadCrumbsEl = Ext.get(parentEl.childNodes[1]);
					var breadCrumbsPrefix = "breadcrumbs";
					var isBreadCrumbsEl = Ext.String.startsWith(breadCrumbsEl.id, breadCrumbsPrefix);
					if (isBreadCrumbsEl && breadCrumbsEl.getStyle("display") !== "none") {
						titleHeight += breadCrumbsEl.getHeight() + 25;
					}
				}
				var chartHeight = pick(optionsChart.height,
						// the offsetHeight of an empty container is 0 in standard browsers, but 19 in IE7:
						chart.containerHeight > 19 ? (chart.containerHeight) : 400) - titleHeight;
				chart.chartHeight = mathMax(0, chartHeight);
			},
