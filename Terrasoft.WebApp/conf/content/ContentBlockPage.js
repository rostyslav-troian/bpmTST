...
define('ContentBlockPageStructure', ['ContentBlockPageResources'], function(resources) {return {schemaUId:'927abe10-66bb-4658-99bf-5b3f80b3f1cf',schemaCaption: "Section edit page schema - \"Content block\"", parentSchemaName: "BaseModulePageV2", schemaName:'ContentBlockPage',parentSchemaUId:'d62293c0-7f14-44b1-b547-735fb40499cd',extendParent:false,type:Terrasoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA,entitySchema:'',name:'',extend:'Terrasoft.model.BaseViewModel',schema:{leftPanel:[],rightPanel:[],actions:[],analytics:[]},methods:{},controlsConfig:{},customBindings:{},bindings:{},schemaDifferences:function(){
...
define("ContentBlockPage", [
	...
	return {
		...
		methods: {
			 ...
			 */
			initEntity: function() {
				if (Terrasoft.Features.getIsDisabled("ContentBlockDesigner")) {
					if (this.isAddMode()) {
			...
			saveEntity: function(callback, scope) {
				if (Terrasoft.Features.getIsEnabled("ContentBlockDesigner")) {
					...
					const blockConfig = contentBuilderHelper.toJSON(this);
					this.set("Config", Terrasoft.encode(blockConfig));
					this.callParent(arguments);
				}
			},
			...
			initBlock: function(blockConfig) {
				if (Terrasoft.Features.getIsEnabled("ContentBlockDesigner")) {
					...
					const contentBuilderHelper = this.createContentBuilderHelper();
					const viewModel = contentBuilderHelper.toViewModel(blockConfig);
					Terrasoft.each(viewModel.values, function(value, key) {
						if (Terrasoft.instanceOfClass(value, "Terrasoft.Collection")) {
							const collection = this.get(key);
							if (collection) {
								collection.reloadAll(value);
							} else {
								this.set(key, value);
							}
						} else if (key !== "Id") {
			...
			getIsVisibleBlankSlate: function() {
				return !Terrasoft.Features.getIsDisabled("ContentBlockDesigner") &&
					this.isAddMode() && !this.get("IsBlockContentBuilderModuleVisible");
			}
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					...
					"items": "Items",
					"dragActionsCode": 0,
					"visible": Terrasoft.Features.getIsDisabled("ContentBlockDesigner")
				}
			},
			{
				"operation": "insert",
