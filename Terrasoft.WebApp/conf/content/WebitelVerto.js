Terrasoft.configuration.Structures["WebitelVerto"] = {innerHierarchyStack: ["WebitelVerto"]};
// jscs:disable
/* jshint ignore:start */
/*ignore jslint start*/
...
define("WebitelVerto", [], function() {
	...
	(function($) {
		...
		$.evalJSON = typeof JSON === 'object' && JSON.parse ? JSON.parse : function() {
			throw new SyntaxError('Error parsing JSON, JSON.parse function is not supported.');
		};
		$.secureEvalJSON = typeof JSON === 'object' && JSON.parse ? JSON.parse : function() {
			throw new SyntaxError('Error parsing JSON, JSON.parse function is not supported.');
		};
	...
	(function($) {
		...
		$.FSRTC = function(options) {
			this.options = $.extend({useVideo: null,useStereo: false,userData: null,localVideo: null,screenShare: false,useCamera: "any",iceServers: false,videoParams: {},audioParams: {},callbacks: {onICEComplete: function() {
			},onICE: function() {
			},onOfferSDP: function() {
			}},}, options);
			this.enabled = true;
			...
			if (moz) {
				this.constraints = {offerToReceiveAudio: true,offerToReceiveVideo: this.options.useVideo ? true : false,};
			} else {
				this.constraints = {optional: [{'DtlsSrtpKeyAgreement': 'true'}],mandatory: {OfferToReceiveAudio: true,OfferToReceiveVideo: this.options.useVideo ? true : false,}};
			}
		...
		function checkCompat() {
			if (!navigator.getUserMedia) {
				console.error('This application cannot function in this browser.');
				return false;
			}
			return true;
		}
		...
		$.FSRTC.prototype.stop = function() {
			...
			if (self.options.useVideo) {
				self.options.useVideo.style.display = 'none';
				self.options.useVideo[moz ? 'mozSrcObject' : 'src'] = moz ? null : "";
			}
			if (self.options.localVideo) {
				self.options.localVideo.style.display = 'none';
				self.options.localVideo[moz ? 'mozSrcObject' : 'src'] = moz ? null : "";
			}
			if (self.localStream) {
		...
		$.FSRTC.prototype.addCustomStream = function() {
			var self = this;
			var screen = false;
			self.type = "offer";
			self.options.useVideo = true;

			function onSuccess(stream) {
				self.localStream = stream;
				self.peer = RTCPeerConnection({type: self.type,attachStream: self.localStream,onICE: function(candidate) {
				...
				},constraints: {audio: false,video: {optional: []},}});
			...
			getUserMedia({constraints: {audio: false,video: {optional: []},},localVideo: null,onsuccess: onSuccess,onerror: function(e) {
		...
		$.FSRTC.prototype.createAnswer = function(params) {
			...
			// TODO fix moz 13.05
			if (moz && !params.useVideo) {
				//self.remoteSDP = self.remoteSDP.replace(/(\nm=video.*)/g, '');
				self.remoteSDP = self.remoteSDP.replace('m=video', 'm1=video');
			};
			...
			function onSuccess(stream) {
				...
				self.peer = RTCPeerConnection({type: self.type,attachStream: self.localStream,onICE: function(candidate) {
					...
					return onChannelError(self, e);
				},constraints: self.constraints,iceServers: self.options.iceServers,offerSDP: {type: "offer",sdp: self.remoteSDP}});
			...
			/* if (self.options.useVideo && self.options.localVideo) {
			 getUserMedia({constraints: {audio: false,video: {mandatory: self.options.videoParams,optional: []},},localVideo: self.options.localVideo,onsuccess: function(e) {
			...
			getUserMedia({constraints: {audio: mediaParams.audio,video: mediaParams.video},video: mediaParams.useVideo || false,onsuccess: onSuccess,onerror: onError});
		};
		function getMediaParams(obj) {
			...
			if (obj.options.videoParams && obj.options.screenShare) {
				...
				audio = {mandatory: obj.options.audioParams,optional: []};
				if (obj.options.useMic !== "any") {
					audio.optional = [{sourceId: obj.options.useMic}]
				}
			}
			if (obj.options.useVideo && obj.options.localVideo) {
				getUserMedia({constraints: {audio: false,video: {mandatory: obj.options.videoParams,optional: []},},localVideo: obj.options.localVideo,onsuccess: function(e) {
					obj.options.localVideoStream = e;
					console.log("local video ready");
					obj.options.localVideo.play();
				},onerror: function(e) {
					console.error("local video error!");
				}});
			}
		...
		$.FSRTC.prototype.call = function(profile) {
			...
			function onSuccess(stream) {
				...
				self.peer = RTCPeerConnection({type: self.type,attachStream: self.localStream,onICE: function(candidate) {
					...
					return onChannelError(self, e);
				},constraints: self.constraints,iceServers: self.options.iceServers,});
				onStreamSuccess(self, stream);
			}
			function onError(e) {
				onStreamError(self, e);
		...
		function RTCPeerConnection(options) {
			var w = window, PeerConnection = w.mozRTCPeerConnection || w.webkitRTCPeerConnection, SessionDescription = w.mozRTCSessionDescription || w.RTCSessionDescription, IceCandidate = w.mozRTCIceCandidate || w.RTCIceCandidate;
			var STUN = {url: !moz ? 'stun:stun.l.google.com:19302' : 'stun:23.21.150.121'};
			...
			if (options.iceServers) {
				...
				iceServers = {iceServers: tmp || [STUN]};
				if (!moz && !tmp) {
			...
			if (!moz) {
				optional.optional = [{DtlsSrtpKeyAgreement: true}, {RtpDataChannels: options.onChannelMessage ? true : false}];
			};
			...
			var constraints = options.constraints || {offerToReceiveAudio: true,offerToReceiveVideo: true};
			...
			function getInteropSDP(sdp) {
				var chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split(''), extractedChars = '';
				function getChars() {
			...
			function _openOffererChannel() {
				channel = peer.createDataChannel(options.channel || 'RTCDataChannel', moz ? {} : {reliable: false});
				if (moz)
			...
			return {addAnswerSDP: function(sdp, cbSuccess, cbError) {
				peer.setRemoteDescription(new SessionDescription(sdp), cbSuccess ? cbSuccess : onSdpSuccess, cbError ? cbError : onSdpError);
			},addICE: function(candidate) {
				peer.addIceCandidate(new IceCandidate({sdpMLineIndex: candidate.sdpMLineIndex,candidate: candidate.candidate}));
		...
		function getUserMedia(options) {
			...
			function streaming(stream) {
				// TODO
				if (moz && options.constraints['video'] && stream.getVideoTracks().length === 0 && options.onerror) {
					stream.stop();
					options.onerror(new Error('DevicesNotFoundError'));
					return;
				};
		...
		var checkRes = function(cam, func) {
			...
			h = resList[resI][1];
			resI++;
			video.mandatory = {"minWidth": w,"minHeight": h,"maxWidth": w,"maxHeight": h};
			getUserMedia({constraints: {audio: true,video: video},onsuccess: function(e) {
				e.stop();
		...
		$.FSRTC.checkPerms = function() {
			getUserMedia({constraints: {audio: true,video: true,},onsuccess: function(e) {
				e.stop();
	...
	(function($) {
		$.JsonRpcClient = function(options) {
			var self = this;
			this.options = $.extend({ajaxUrl: null,socketUrl: null,onmessage: null,login: null,passwd: null,sessid: null,loginParams: null,getSocket: function(onmessage_cb) {
		...
		$.JsonRpcClient.prototype.call = function(method, params, success_cb, error_cb) {
			...
			$.ajax({type: 'POST',url: this.options.ajaxUrl,data: $.toJSON(request),dataType: 'json',cache: false,success: function(data) {
				if ('error' in data)
				...
				if (typeof response === 'object' && 'jsonrpc' in response && response.jsonrpc === '2.0') {
					if ('result' in response && this._ws_callbacks[response.id]) {
						...
						if (!self.authing && response.error.code == -32000 && self.options.login && self.options.passwd) {
							self.authing = true;
							this.call("login", {login: self.options.login,passwd: self.options.passwd,loginParams: self.options.loginParams}, this._ws_callbacks[response.id].request_obj.method == "login" ? function(e) {
								self.authing = false;
		...
		$.JsonRpcClient._batchObject.prototype._execute = function() {
			...
			$.ajax({url: self.jsonrpcclient.options.ajaxUrl,data: $.toJSON(batch_request),dataType: 'json',cache: false,type: 'POST',error: function(jqXHR, textStatus, errorThrown) {
	...
	(function($) {
		...
		$.verto = function(options, callbacks) {
			...
			$.cookie('verto_session_uuid', verto.sessid, {expires: 1});
			verto.dialogs = {};
			verto.callbacks = callbacks || {};
			verto.eventSUBS = {};
			verto.rpcClient = new $.JsonRpcClient({login: verto.options.login,passwd: verto.options.passwd,socketUrl: verto.options.socketUrl,loginParams: verto.options.loginParams,sessid: verto.sessid,onmessage: function(e) {
		...
		$.verto.confMan = function(verto, params) {
			...
			function genMainMod(jq) {
				...
				if (confMan.params.hasVid) {
					...
					var vlhtml = "<div id='" + vlayout_id + "'><br>" + "<b>Video Layout</b> <select id='" + vlselect_id + "'></select> " + "<br><br></div>";
					jq.append(vlhtml);
			...
			if (confMan.params.laData.role === "moderator") {
				...
				verto.subscribe(confMan.params.laData.modChannel, {handler: function(v, e) {
			...
			confMan.lt = new $.verto.liveTable(verto, confMan.params.laData.laChannel, confMan.params.laData.laName, $(confMan.params.tableID), {subParams: {callID: confMan.params.dialog ? confMan.params.dialog.callID : null},"onChange": function(obj, args) {
		...
		$.verto.dialog = function(direction, verto, params) {
			...
			RTCcallbacks.onICESDP = function(rtc) {
				if (rtc.type == "offer") {
					// TODO fix moz
					/*if (moz) {
					 rtc.mediaData.SDP = rtc.mediaData.SDP.replace(/(\nm=video.*)/g, '');
					 };*/
			...
			RTCcallbacks.onError = function(e, errOb) {
				console.error("ERROR:", e);
				dialog.hangup({cause: "Device or Permission Error"});
				if (dialog.callbacks.onError) {
					dialog.callbacks.onError.apply(dialog, [dialog, errOb]);
				}
			};
			dialog.rtc = new $.FSRTC({callbacks: RTCcallbacks,localVideo: dialog.localVideo,useVideo: dialog.videoStream,useAudio: dialog.audioStream,useStereo: dialog.params.useStereo,videoParams: dialog.params.videoParams,audioParams: verto.options.audioParams,iceServers: verto.options.iceServers,screenShare: dialog.screenShare,useCamera: dialog.useCamera,useMic: dialog.useMic});
		...
		$.verto.dialog.prototype.fsapi = function(cmd, arg) {
			var dialog = this;
			dialog.sendMethod("fsapi", {params: {cmd: cmd, arg: arg}});
		};
		...
		$.verto.enum = {};
		$.verto.enum.states = Object.freeze({new: {requesting: 1,recovering: 1,ringing: 1,destroy: 1,answering: 1,hangup: 1},requesting: {trying: 1,hangup: 1},recovering: {answering: 1,hangup: 1},trying: {active: 1,early: 1,hangup: 1},ringing: {answering: 1,hangup: 1},answering: {active: 1,hangup: 1},active: {answering: 1,requesting: 1,hangup: 1,held: 1},held: {hangup: 1,active: 1},early: {hangup: 1,active: 1},hangup: {destroy: 1},destroy: {},purge: {destroy: 1}});
