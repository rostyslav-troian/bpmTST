Terrasoft.configuration.Structures["MainMenuTileGenerator"] = {innerHierarchyStack: ["MainMenuTileGenerator"]};
define("MainMenuTileGenerator", ["ext-base", "terrasoft", "ModuleUtils", "ViewGeneratorV2"
], function(Ext, Terrasoft, ModuleUtils) {
	...
	Ext.define("Terrasoft.configuration.MainMenuTileGenerator", {
		 ...
		 * @param {Object} config.windowsIcon Windows icon config.
		 * @param {String} config.windowsUrl Windows app url.
		 * @param {String} config.windowsLabel Windows app label.
		...
		generateMobileAppBaner: function(config) {
			var itemsInfo = [
				...
				},
				{
					...
					icon: config.windowsIcon,
					label: config.windowsLabel
				}
			];
