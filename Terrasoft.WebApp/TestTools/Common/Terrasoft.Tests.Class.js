	...
	my: {
		methods: {
			addBDDAssertion: function(builder, test) {
				if (!builder.isEmpty()) {
					if (test.harness.isPhantomJS && test.harness.writeToFileOnNotBDD) {
						test.addResult(new Siesta.Result.DiagnosticV2({
							description: String(builder.toString() || ""),
							isSpecForBDD: true
						}));
					}
					if (!test.harness.isPhantomJS && test.harness.warnOnNotBDD) {
...
Class("Terrasoft.Test.Class", {
	...
	override: {
		...
		diag: function(message, isUseOriginal) {
			...
			if (queue.length === 0) {
				if (this.checkCondition(message, this.testFilter)){
					this.SUPERARG([message]);
				}
			} else {
		...
		finalize: function() {
			...
			try {
				...
				var failMsg = "tearDown failed with exception - " + e.message;
				if (testObj){
					failMsg = testObj.name + ": " + failMsg;
				}
				this.fail(failMsg, e.name);
			}
			if (this.finalizationStarted && this.isFinished()) {
				this.clearAfterTest();
			}
	...
	methods: {
		...
		testMethod: function(testObj) {
			...
			var queue = this.queue;
			var message = queue[0];
			if (this.checkCondition(testObj.name, this.testFilter)) {
				queue.push(testObj);
			}
			if (queue.length === 1 && (!this.testFilter || !this.testsRunning)) {
 				this.runNextTest();
 			}
		},

		checkCondition: function(value, regexp){
			if (!regexp) {
				return true;
			}
		...
		testMethodDone: function() {
			...
			if (typeof testObj === "object") {
				...
				if (typeof tearDown === "function") {
					try {
						tearDown.call(this, function() {}, function() {});
					} catch (e) {
						var testObj = this.queue[0];
		 ...
		 * @param {Object} expectedPropertyValues Expected propery values.
		...
		/**
		 * Creates proxy object. Useful for when modificaton of global object is need.
		 * @param {Object} instance Instance to proxy.
		 */
		createProxy: function(instance) {
			var constructor = function() {};
			constructor.prototype = instance;
			constructor.constructor = constructor;
			constructor.superclass = instance;
