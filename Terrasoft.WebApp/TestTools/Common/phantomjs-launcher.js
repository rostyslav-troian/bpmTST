...
var runPage = function (iface, params, callback) {
    ...
    iface.onPageExit = function () {
        clearInterval(timeoutCheckerId)
			...
			console.log('------------------------------ closing and saving report----------------------');
			var fs = require('fs');
			var json = String(iface.executeScript('return (window.jscoverage_serializeCoverageToJSON != null) ? window.jscoverage_serializeCoverageToJSON() : {};'));
			var jsCoverReportPath = options['jscover-reportpath'];
			if (jsCoverReportPath) {
				if (jsCoverReportPath.lastIndexOf('\\') !== jsCoverReportPath.length - 1) {
					jsCoverReportPath += '\\';
				}
				jsCoverReportPath += fileVersion.toString() + '\\';
				var reportFileName = 'jscoverage.json';
				fileVersion++;
				fs.write(jsCoverReportPath + reportFileName, json, 'w');
			}
